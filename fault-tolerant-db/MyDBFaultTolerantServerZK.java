...
public class MyDBFaultTolerantServerZK extends server.MyDBSingleServer implements Watcher {
	...
	private void recoverFromCrash() throws KeeperException, InterruptedException {
		...
		for (Row row : resultSet) {
			//System.out.println("ROW What is going on ------------------------------>"+row.getInt("id"));
		...
		for (String child : children) {
			...
			if (!idsInCassandra.contains(child)) {// Check if the event already exists in Cassandra
				if (!event.isEmpty()) {
					...
					System.out.println(myID + " Recovering data for znode: " + reconstructedSQL + "  Child:" + child);
				} else {
					System.out.println(myID + " Skipping update for existing event: " + event + " for ID: " + child);
				}
			}
			// Execute update in Cassandra
		}
	...
	private void processMessagesFromQueue() throws InterruptedException {
		while (!Thread.currentThread().isInterrupted()) {
			...
			if (request != null) {
				// Existing logic to process the request
				// For example, handleInsertRequest or handleUpdateRequest
				try {
	...
	private void releaseLock(int id) throws KeeperException, InterruptedException {
		...
		try {
			...
			// The node doesn't exist, it may have been already released or expired
		}
	}
	...
	private byte[] updateEventData(String currentData, String request) {
		...
		//System.out.println("[" +currentData+", "+ extractEventFromUpdate(request) + "]");
	...
	private void handleEvent(WatchedEvent event) {
		// Implement the logic to handle the event
		// This method will be executed in a separate thread
		// Example: eventQueue.offer(event);

		//System.out.println(myID+" ENUEUE  "+event);
	...
	private void processEvents() throws InterruptedException {
		while (!Thread.currentThread().isInterrupted()) {
			...
			if (event.getType() == Event.EventType.NodeChildrenChanged && event.getPath().equals(DATA_ZNODE)) {
				//System.out.println("Child Change Detected !!!");
				...
				//setParentNodeWatch(); // Reset the watch
			}
			...
			if (event.getType() == Event.EventType.NodeDataChanged) {
				...
				try {
					...
					//System.out.println(myID+"  Change in the Znode data ------- >>>>>>>> "+dataStr);
					...
					//System.out.println(myID+"  ------------------------->"+ Arrays.toString(numbers));
					...
					String numberString = stringBuilder.toString();
						...
						//System.out.println("UPDATING THE DB--------------->"+numberString);
